...
def main(saved_root,split_root,data_root,region_table_path,
    ...
    COMMAND = "python3 "+MAIN_PATH+" -s {} -d {} -r {} --answer_gff_path {} --region_table_path {}"
    ...
    for project_name,paths in data_names.items():
        ...
        for type_,name in paths.items():
            ...
            gff3_path = os.path.join(split_root,"gff",name+"_canonical.gff")
            ...
            command = COMMAND.format(project_path,h5_path,result_root,gff3_path,region_table_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-r","--saved_root",required=True,help='The path root generated by batch_train_command_generator')
    parser.add_argument("-s","--split_root",help='The data folder about split data',required=True)
    parser.add_argument("-d","--data_root",help='The data folder generated by batch_select_data.py',required=True)
    parser.add_argument("--save_command_table_path",help='The path to save batch commands in tsv format',required=True)
    parser.add_argument("--region_table_path",required=True)
    parser.add_argument("--converter_args_path",type=str,help="The path about keyword arguments of build_ann_vec_gff_converter")
