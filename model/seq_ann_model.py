...
class CleanLogger(BaseLogger):
    ...
    def __init__(self):
        ...
        self.seen = None
        self.totals = None
        self.each_count = None
    def on_epoch_begin(self, epoch, logs=None):
        """Initialize when some variable when epoch begin"""
        self.seen = 0
        self.totals = {}
        self.each_count = {}
    def on_batch_end(self, batch, logs=None):
        """Stored and calcualte some metric when batch end"""
        logs = logs or {}
        batch_size = logs.get('size', 0)
        self.seen += batch_size
        for key, value in logs.items():
            if key not in self.each_count.keys():
                self.each_count[key] = 0
                self.totals[key] = 0
            if not np.isnan(value):
                self.each_count[key] += batch_size
                self.totals[key] += value * batch_size
    def on_epoch_end(self, epoch, logs=None):
        """Stored and calcualte some metric when epoch end"""
        if logs is not None:
            for key in self.params['metrics']:
                if key in self.totals:
                    # Make value available to next callbacks.
                    if self.each_count[key] != 0:
                        logs[key] = self.totals[key] / self.each_count[key]
                    else:
                        logs[key] = np.float64('nan')
class SeqAnnModel(Model):
    """A class inherits to Model which can handler advanced usage"""
            ...
            if steps_per_epoch is not None:
                for step_index in range(steps_per_epoch):
                    ...
                    if not isinstance(outs, list):
                        outs = [outs]
                    for label, out in zip(out_labels, outs):
                        batch_logs[label] = out

                    callbacks.on_batch_end(step_index, batch_logs)
                    if callback_model.stop_training:
                        break
                if do_validation:
                    val_outs = self._test_loop(val_f, val_ins, batch_size=batch_size,
                                               steps=validation_steps, verbose=0)
                    if not isinstance(val_outs, list):
                        val_outs = [val_outs]
                    # Same labels assumed.
                    for label, out in zip(out_labels, val_outs):
                        epoch_logs['val_' + label] = out
            else:
                if shuffle == 'batch':
