...
def get_region_around_site(gff,feature_types,upstream_distance,downstream_distance,is_start_location=True):
...
def get_transcription_start_site_region(gff,upstream_distance,downstream_distance):
    data = get_region_around_site(gff,RNA_TYPES,upstream_distance,downstream_distance)
    return data
...
def get_donor_site_region(gff,upstream_distance,downstream_distance):
    data = get_region_around_site(gff,INTRON_TYPES,upstream_distance,downstream_distance)
    return data
...
def main(gff_path,output_root,tss_radius,cleavage_radius,donor_radius,acceptor_radius):
    ...
    #print("get site")
    tss_region = get_transcription_start_site_region(gff,tss_radius,tss_radius)
    ...
    tss_signal_region = get_transcription_start_site_region(gff,TSS_SIGNAL_RADIUS,TSS_SIGNAL_RADIUS)
    cs_signal_region = get_cleavage_site_region(gff,CS_SIGNAL_RADIUS,CS_SIGNAL_RADIUS)
    donor_signal_region = get_donor_site_region(gff,1,1)
    acceptor_signal_region = get_acceptor_site_region(gff,2,0)
    
    regions = [tss_region,cs_region,donor_region,acceptor_region,
               tss_signal_region,cs_signal_region,
               donor_signal_region,acceptor_signal_region]
    
    names = ["tss_around_{}".format(tss_radius),"cleavage_site_around_{}".format(cleavage_radius),
...
if __name__ =='__main__':
    parser = ArgumentParser(description="This program will read gff file and output block information in BED format")
    parser.add_argument("-i", "--gff_path", help="Path of GFF file",required=True)
    parser.add_argument("-o", "--output_root", help="Root to save data",required=True)    
