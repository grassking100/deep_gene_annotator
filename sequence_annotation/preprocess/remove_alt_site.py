...
from sequence_annotation.preprocess.path_decode import _get_most_start_end_with_transcripts
from sequence_annotation.preprocess.path_decode import _get_canonical_region_and_splice_site
...
def get_no_alt_sie_transcript_id(bed_path,relation_path,select_site_by_election=False):
    ...
    for gene_id,mRNAs in genes.items():
        try:
            ...
            if len(mRNAs) > 0:
                _,alt_ds,alt_as,d_a_site = _get_canonical_region_and_splice_site(mRNAs,start_sites,end_sites)
