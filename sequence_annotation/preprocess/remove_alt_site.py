...
def get_no_alt_site_transcript_id(bed_path,relation_path,select_start_end_site_by_election=False):
    ...
    for gene_id,mRNAs in genes.items():
        try:
            if select_start_end_site_by_election:
                mRNAs = get_most_start_end_transcripts(start_sites,end_sites,mRNAs)
            if len(mRNAs) > 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--select_start_end_site_by_election",action='store_true')
    args = parser.parse_args()
    ids = get_no_alt_site_transcript_id(args.input_bed_path,args.id_table_path,
                                        select_start_end_site_by_election=args.select_start_end_site_by_election)
