...
if __name__ == "__main__":
    ...
    for item in bed.to_dict('record'):
        if item['thick_length'] < 0:
            ...
            gene_id_has_noncoding_transcript.add(id_convert_table[item['id']])

    all_coding_transcript_gene_transcripts = []
    for item in bed.to_dict('record'):
        gene_id = id_convert_table[item['id']]
        if gene_id not in gene_id_has_noncoding_transcript:
            all_coding_transcript_gene_transcripts.append(item)
    all_coding_transcript_gene_transcripts = pd.DataFrame.from_dict(all_coding_transcript_gene_transcripts)
