...
from sequence_annotation.preprocess.site_analysis import get_stats

def coordinate_compare(compared,comparing):
...
def main(reference_bed_path,redefined_bed_path,output_path):
    ...
    reference_as_ref = coordinate_compare(reference,redefined)
    redefined_as_ref = coordinate_compare(redefined,reference)
    result = {}
    ...
    pd.DataFrame.from_dict(result).to_csv(output_path)
