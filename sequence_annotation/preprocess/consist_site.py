...
def _read(path):
    ...
    coord_ref_id = gff['chr'].astype(str)+"_"+gff['strand'].astype(str)+"_"+gff['start'].astype(str)+"_"+gff['ref_name'].astype(str)
    coord_id = gff['chr'].astype(str)+"_"+gff['strand'].astype(str)+"_"+gff['start'].astype(str)
...
def get_ophan_on_ld_data(on_long_dist,on_external_UTR):
    return on_long_dist[~on_long_dist['coord_id'].isin(on_external_UTR['coord_id'])]
...
def get_consist_site(on_external_UTR,on_long_dist,on_transcript,UTR_target,remove_conflict):
    #Create orhpan data
    ...
    data = consist(data,'ref_name','experimental_score',drop_duplicated=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--remove_transcript_external_UTR_conflict",action='store_true')
    ...
    if all(exists):
        ...
        consist_tss_site = get_consist_site(external_five_UTR_tss,ld_tss,transcript_tss,"external_5_UTR_TSS",
                                            args.remove_transcript_external_UTR_conflict)
        consist_cs = get_consist_site(external_three_UTR_cs,ld_cs,transcript_cs,'external_3_UTR_CS',
                                      args.remove_transcript_external_UTR_conflict)

        ###Write data###)
        write_gff(get_gff_with_updated_attribute(consist_tss_site),safe_tss_path)
