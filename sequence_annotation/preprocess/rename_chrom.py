...
def rename_chrom(data, table, source,target,type_):
    renamed = dict(zip(list(table[source]),list(table[target])))
...
def main(input_path, table_path, output_path,source,target):
    table = read_region_table(table_path)
    ...
    renamed = rename_chrom(data, table,source,target,type_)
    if 'gff3' == type_:
