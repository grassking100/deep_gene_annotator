...
def select_data_by_length_each_type(fasta, ann_seqs,min_len=None,
    ...
    for subfasta, sub_ann_seqs in zip(fasta_list, ann_list):
        data = select_data_by_length(subfasta,sub_ann_seqs,
...
def _select_data_by_name(fasta_path,ann_seqs_path,id_path,output_path,min_len=None,max_len=None,
                         ratio=None,select_each_type=False,input_gff_path=None,output_gff_path=None):
    min_len = min_len or 0
    ratio = ratio  or 1
    ...
    if input_gff_path is not None and output_gff_path is not None:
        ...
        selected_gff = gff[gff['chr'].isin(ids)]
        write_gff(selected_gff,output_gff_path)  
            
def main(fasta_path,ann_seqs_path, usage_table_path,output_root,
         name=None,**kwargs):
    setting = locals()
    create_folder(output_root)
    usage_table_root = '/'.join(usage_table_path.split('/')[:-1])
    if name is None:
        path = os.path.join(output_root, "batch_select_data_config.json")
        output_path = os.path.join(output_root,'split_table.csv')
        ...
        for dataset in usage_table:
            ...
            for type_, path in dataset.items():
                ...
                output_paths[type_] = '{}.h5'.format(name)
                output_path = os.path.join(output_root, output_paths[type_])
                _select_data_by_name(fasta_path,ann_seqs_path,id_path,output_path,**kwargs)
            output_table.append(output_paths)
        pd.DataFrame.from_dict(output_table).to_csv(output_path,index=None)
        write_json(setting, path)
    else:
        output_path = os.path.join(output_root, "select_data_{}_config.json".format(name))
        id_path = os.path.join(usage_table_root,"{}.txt".format(name))
        output_path = os.path.join(output_root, '{}.h5'.format(name))
        _select_data_by_name(fasta_path,ann_seqs_path,id_path,output_path,**kwargs)
        write_json(setting, output_path)

if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument("-f", "--fasta_path", required=True,help="Path of fasta")
    parser.add_argument("-a", "--ann_seqs_path", required=True,help="Path of AnnSeqContainer")
    parser.add_argument("-o", "--output_root", required=True,help="Root to save file")
    parser.add_argument("-u", "--usage_table_path", required=True,help="Usage table in csv format")
