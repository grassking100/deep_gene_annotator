    ...
    if transcript_id in list(exon_group.groups.keys()):
        ...
        if transcript_id in list(subexon_group.groups.keys()):
            ...
            if len(exons) != len(created_exons):
                raise Exception("Inconsist exon number at {}, got {} and {}".format(transcript_id,len(exons),
                                                                                    len(created_exons)))
...
def repair_gff(gff):
    ...
    for index,transcript_id in enumerate(list(transcripts['id'])):
        ...
        subexons_list_,exon_list_ = _get_repaired_subexon_and_exon(transcript_id,exon_group,subexon_group)
    ...
    for index,transcript in enumerate(transcript_list):
        print("Processed {}%".format(int(100*index/len(transcript_list))),end='\r')
        sys.stdout.write('\033[K')
        transcript_id = transcript['id']
        if transcript_id in exon_list_group:
            matched_list = exon_list_group[transcript_id]
            transcript = _get_item_with_repaired_boundary(transcript,matched_list)
...
if __name__ =='__main__':
    ...
    parser.add_argument("-s", "--saved_root",help="Root to save result",required=True)
