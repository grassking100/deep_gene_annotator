...
if __name__ == "__main__":
    ...
    cleavage_site_gff_path = os.path.join(args.output_root,'cleavage_site.gff3')
    ...
    if all(exists):
        ...
        print("The number of GRO are {} and {},the common one is {}".format(len(gro_1),len(gro_2),len(gro)))
        ...
        gro = gro.drop('end', 1)
        gro = gro.assign(id=gro.astype(str).apply(lambda x: '_'.join(x), axis=1))
        ...
        ca_site.columns = ['chr','strand','evidence_3_end','experimental_score']
        ca_site.loc[:,'chr'] = ca_site['chr'].str.replace('chr','')
        ca_site.loc[ca_site['strand']=='+','evidence_3_end'] += 1
        ca_site.loc[ca_site['strand']=='-','evidence_3_end'] -= 1
        ca_site = ca_site.assign(id=ca_site.astype(str).apply(lambda x: '_'.join(x), axis=1))
        ###Drop duplicated ###
