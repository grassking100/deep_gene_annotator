...
from sequence_annotation.utils.utils import read_bed, write_bed, read_gff, read_fasta
...
def create_bed_with_orf(bed,cDNAs,start_codons,stop_codons):
    returned_list = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input_path",help='If postfix is bed, then it will be used directly'+
                        '. If postfix is gff or gff3, it will be converted to bed first',required=True)
    parser.add_argument("-o", "--output_bed_path",required=True)
    parser.add_argument("-f","--fasta_path")
    parser.add_argument("-t","--translation_table_path",help="If it is not provided, then standard codon will be used")
    ...
    cDNA_fasta_path = '.'.join(args.input_path.split('.')[:-1])+"_cDNA.fasta"
    command = 'bedtools getfasta -split -name -s -fi {} -bed {} -fo {}'
    command = command.format(args.fasta_path,args.input_path,cDNA_fasta_path)
    print(command)
    os.system(command)
    ...
    bed = read_bed(args.input_path)
    returned_bed = create_bed_with_orf(bed,cDNAs,start_codons,stop_codons)
    write_bed(returned_bed,args.output_bed_path)
