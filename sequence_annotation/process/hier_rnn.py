...
class HierRNN(BasicModel):
    def __init__(self, preprocess, rnn_0, rnn_1, hier_option=None):
        ...
        if self.hier_option not in [
                'before_filter', 'after_filter', 'independent'
        ]:
    ...
    def forward(self, x, lengths, answers=None, **kwargs):
        if self.preprocess is not None:
            x = self.preprocess.forward(x, lengths)
...
class HierRNNBuilder:
    ...
    def _create_rnn(self, use_filter, name):
        if use_filter:
            rnn = FilteredRNN(self._in_channels,
                              out_channels=1,
                              name=name,
                              filter_num_layers=self._filter_num,
                              filter_hidden_size=self._filter_hidden,
                              **self._kwargs)
        else:
            rnn = ProjectedRNN(self._in_channels,
                               out_channels=1,
                               name=name,
                               **self._kwargs)
        return rnn
    ...
    def _create_rnn_filter(self, name):
        rnn = RNNFilter(self._in_channels,
                        name=name,
                        num_layers=self._filter_num,
                        hidden_size=self._filter_hidden,
                        **self._kwargs)
        return rnn
