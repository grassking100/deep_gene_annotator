    ...
    def on_epoch_end(self, **kwargs):
        ...
        if self.do_add_distribution and hasattr(self._model,
            ...
            for name, value in self._model.saved_distribution.items():
                self._writer.add_distribution(name,
                                              value,
                                              prefix=self._prefix,
                                              counter=self._counter)
                ...
                self._writer.add_figure(name + "_figure",
                                        value,
                                        prefix=self._prefix,
                                        counter=self._counter,
                                        title=name)
        ...
        self._writer.add_figure("result_figure",
                                onehot,
                                prefix=self._prefix,
                                colors=self.colors,
                                labels=self.label_names,
        ...
        self._writer.add_figure("diff_figure",
                                diff,
                                prefix=self._prefix,
                                colors=self.colors,
                                labels=self.label_names,
                                title="Predict - Answer figure",
                                use_stack=False)
...
class CategoricalMetric(DataCallback):
    ...
    def label_names(self, names):
        if len(names) != self.label_num:
            raise Exception(
                'The number of class\'s name is not the same with class\' number'
            )
    ...
    def on_batch_end(self, seq_data, **kwargs):
        ...
        for name, val in self._model.saved_distribution.items():
            if self.saved_distribution_name in name:
                signals = self._model.saved_distribution[name].cpu().detach(
                ).numpy()
        ...
        for name, signal in self._signals.items():
            ...
            if ppm is not None:
                ...
                if len(valid_loc) > 0:
                    ...
                    ppm_df = pd.DataFrame(ppm.T, columns=list(DNA_CODES))
                    seqlogo_ppm = seqlogo.Ppm(ppm_df,
                                              background=background_freq)
                    # Save seqlogo
                    seqlogo.seqlogo(seqlogo_ppm,
                                    ic_scale=True,
                                    format='png',
                                    size='xlarge',
