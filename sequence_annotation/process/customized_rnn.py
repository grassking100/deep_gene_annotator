...
def _reverse(x, lengths):
    # Convert forward data data to reversed data with N-C-L shape to N-C-L
    # shape
    x = x.transpose(1, 2)
    N, L, C = x.shape
...
class BidirectionalRNN(BasicModel):
    def __init__(self, forward_rnn, backward_rnn):
        ...
        if self.forward_rnn.out_channels != self.backward_rnn.out_channels:
            raise Exception(
                "Forward and backward RNNs' out channels should be the same")
...
class ConcatRNN(BasicModel):
    def __init__(self, in_channels, hidden_size, num_layers, rnn_type,
                 **rnn_setting):
        super().__init__()
    ...
    def get_config(self):
        ...
        rnn_setting['customized_gru_init'] = str(
            rnn_setting['customized_gru_init'])
