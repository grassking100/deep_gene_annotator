...
from .rnn import GRU, LSTM, customized_init_gru, xavier_uniform_in_
...
class CANBlock(BasicModel):
    def __init__(self,in_channels,kernel_size,out_channels,
                 ...
                 activation_function=None,customized_init=None,padding_handle=None):
        super().__init__()
        self.name = ""
        ...
        self.cnn = Conv1d(in_channels=self.in_channels,kernel_size=self.kernel_size,
                          ...
                          padding_handle=self.padding_handle)
...
class SeqAnnBuilder:
    ...
    def build(self):
        ...
        if 'customized_init' in relation_block_config['rnn_setting']:
            if relation_block_config['rnn_setting']['customized_init']:
                init_func = customized_init_gru
            else:
                init_func = None
            relation_block_config['rnn_setting']['customized_init'] = init_func

        if 'customized_init' in project_layer_config:
            if project_layer_config['customized_init']:
                init_func = customized_init_cnn
            else:
                init_func = None
            project_layer_config['customized_init'] = init_func
