...
class Conv1d(nn.Conv1d, BasicModel):
    def __init__(self,padding_handle=None,padding_value=None,
        ...
        if padding_handle in PADDING_HANDLE:
            ...
            raise Exception(
                "Invalid mode {} to handle padding".format(padding_handle))
        if self._padding_handle != 'valid':
            if self.padding != (0, ) or self.dilation != (
                    1, ) or self.stride != (1, ):
                raise Exception(
                    "The padding_handle sholud be valid to set padding, "
                    "dilation and stride at the same time")

        if kernek_size > 1:
            if self._padding_handle == 'partial' and self.padding_value != 0:
                raise Exception(
                    "When padding_handle is partial, the padding_value should be 0"
                )
            if kernek_size % 2 == 0:
                bound = int(kernek_size / 2)
                self._pad_func = nn.ConstantPad1d((bound - 1, bound),
                                                  self.padding_value)
            else:
                bound = int((kernek_size - 1) / 2)
                self._pad_func = nn.ConstantPad1d((bound, bound),
                                                  self.padding_value)

        self.mask_kernel = torch.ones(
            (self.out_channels, self.in_channels, self.kernel_size[0])).cuda()
    ...
    def forward(self, x, lengths=None, weights=None, mask=None):
        ...
        if self._padding_handle == 'same' or self.kernel_size[0] == 1:
            ...
            new_lengths = [
                math.floor((length + change) / stride) + 1
            ...
            if weights is None:
                warnings.warn(
                    "Caution: weights can be reused ONLY if kernel sizes of previous "
...
class CANBlock(BasicModel):
    def __init__(self,in_channels,norm_mode=None,norm_class=None,
        ...
        self.cnn = Conv1d(in_channels=self.in_channels,
                          bias=use_bias,
                          **kwargs)
...
class StackCNN(BasicModel):
    ...
    def get_config(self):
        ...
        if self.norm_class is not None:
            ...
            config['norm_class_config'] = {
                'kwrags': norm_config['kwrags'],
                'type': norm_config['type']
            }
        return config
    ...
    def __init__(self,
        ...
        for index in range(self.num_layers):
            ...
            if self.bottleneck_factor > 0:
                ...
                bottleneck = CANBlock(in_channels=in_num,
                                      kernel_size=1,
