...
def get_site_diff(answer,predict,types,is_start=True,absolute=True,answer_as_ref=True,include_zero=True):
        ...
        for strand in ['+','-']:
            ...
            if len(answer_sites) > 0 and len(predict_sites) > 0:
                ...
                for ref_site in ref_sites:
                    diffs = [compare_site-ref_site for compare_site in compare_sites]
                    if not include_zero:
...
def get_site_matched_ratio(answer,predict,types,is_start=True,answer_denominator=True):
    site_type = 'start' if is_start else 'end'
    answer = answer[answer['feature'].isin(types)]
    predict = predict[predict['feature'].isin(types)]
    chroms = set(list(answer['chr'])).intersection(set(list(predict['chr'])))
    ...
    for chrom in chroms:
        for strand in ['+','-']:
            answer_group = answer[(answer['chr']==chrom) & (answer['strand']==strand)]
            predict_group = predict[(predict['chr']==chrom) & (predict['strand']==strand)]
            answer_sites += ["{}_{}_{}".format(chrom,strand,site) for site in list(answer_group[site_type])]
            predict_sites += ["{}_{}_{}".format(chrom,strand,site) for site in list(predict_group[site_type])]
...
def get_acceptor_site_matched_ratio(answer,predict,absolute=True,**kwargs):
    return get_site_matched_ratio(answer,predict,['intron'],is_start=False,**kwargs)
...
def get_transcript_end_matched_ratio(answer,predict,absolute=True,**kwargs):
    return get_site_matched_ratio(answer,predict,['mRNA'],is_start=False,**kwargs)

def get_all_site_matched_ratio(answer,predict,round_value=None):
    ratio = {'precision':{},'recall':{},'F1':{}}
    ratio['precision']['TSS'] = get_transcript_start_matched_ratio(answer,predict,answer_denominator=False)
    ratio['precision']['CA'] = get_transcript_end_matched_ratio(answer,predict,answer_denominator=False)
    ratio['precision']['donor_site'] = get_donor_site_matched_ratio(answer,predict,answer_denominator=False)
    ratio['precision']['acceptor_site'] = get_acceptor_site_matched_ratio(answer,predict,answer_denominator=False)

    ratio['recall']['TSS'] = get_transcript_start_matched_ratio(answer,predict)
    ratio['recall']['CA'] = get_transcript_end_matched_ratio(answer,predict)
    ratio['recall']['donor_site'] = get_donor_site_matched_ratio(answer,predict)
    ratio['recall']['acceptor_site'] = get_acceptor_site_matched_ratio(answer,predict)
...
def get_all_site_diff(answer,predict,round_value=None,return_value=False,**kwargs):
...
def main(predict_path,answer_path,saved_root):
    ...
    site_diffs = get_all_site_diff(answer,predict,return_value=True,absolute=False)
    for type_,diffs in site_diffs.items():
        plt.clf()
        plt.title("The distance between predict {} and real {}".format(type_,type_))
        plt.ylabel("Number")
        plt.xlabel("Distance")
        plt.hist(diffs,bins=100,log=True)
        plt.savefig(os.path.join(saved_root,"{}_diff_distribution.png".format(type_)))

if __name__ == '__main__':
    parser = ArgumentParser(description='Show distance between predict GFF to answer GFF')
