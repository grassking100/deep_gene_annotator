...
from sequence_annotation.file_process.utils import BASIC_GENE_ANN_TYPES, PLUS, MINUS
...
def _gene_gff2vec(gff,chrom,strand,length):
    ...
    for block in gff[gff['feature']==GENE_TYPE].to_dict('record'):
        ann_seq.add_ann(GENE_TYPE,1,block['start']-1,block['end']-1)
    for block in gff[gff['feature']==EXON_TYPE].to_dict('record'):
...
def calculate_base_performance(predict,answer,region_table,round_value=None,multiprocess=None):
    ...
    for index in range(len(region_table)):
        chrom = region_table['ordinal_id_wo_strand'][index]
        length = region_table['length'][index]
        strand = region_table['strand'][index]
...
def gff_performance(predict,answer,region_table,round_value=None,multiprocess=None,calculate_base=True):
    """
...
def compare_and_save(predict,answer,region_table,saved_root,**kwargs):
