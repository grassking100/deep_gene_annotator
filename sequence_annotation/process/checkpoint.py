...
class Recorder(DataCallback):
    ...
    def epoch_start(self, value):
        warnings.warn("epoch_start SHOULD only be called by checkpoint",
                      CheckpointProtectedWarning)
    ...
    def _reset(self):
        ...
        if self.path is not None and os.path.exists(
                self.path) and not self._force_reset:
...
class ModelCheckpoint(_Checkpoint):
    def __init__(self,root,target=None,optimize_min=True,
                 patience=None,period=None,save_best_weights=False,
                 restore_best_weights=False,explicit_period=False,
                 warmup_epoch=None):

        super().__init__(root, period=period)
        self.warmup_epoch = warmup_epoch or 0
        ...
        self.latest_status_path = os.path.join(self.root,
                                               'latest_model.status')
    ...
    def paths(self):
        paths = {}
        paths['last'] = {
            'status': self.last_status_path,
            'weights': self.last_model_path
        }

        paths['best'] = {
            'status': self.best_status_path,
            'weights': self.best_model_path
        }

        paths['latest'] = {
            'status': self.latest_status_path,
    ...
    def on_work_begin(self, worker, **kwargs):
        ...
        if os.path.exists(self.latest_status_path):
            biggest_epoch, model_path = _read_status(self.root,
                                                     self.latest_status_path)
        if os.path.exists(self.last_status_path):
            biggest_epoch_, model_path_ = _read_status(self.root,
                                                       self.last_status_path)
            if biggest_epoch_ >= biggest_epoch:
                biggest_epoch = biggest_epoch_
                model_path = model_path_
        if model_path is not None:
            print("Load model of epoch {} from {}".format(
                biggest_epoch, model_path))
            self._worker.model.load_state_dict(torch.load(model_path,
                                                          map_location='cpu'),
                                               strict=True)
        ...
        if self.save_best_weights:
            if os.path.exists(self.best_status_path):
                ...
                print("Load existed best model of epoch {} from {}".format(
                    best_epoch, self.best_model_path))
                print("Load best target, {}, of epoch {} from {}".format(
                    best_target, best_epoch, self.best_status_path))
                if self._counter < self._best_result:
                    raise Exception(
                        "Best epoch {} should not exceed current epoch {}".
                        format(self._best_result, self._counter))
            else:
    ...
    def on_epoch_begin(self, counter, **kwargs):
        ...
        if self._counter > self.warmup_epoch:
    ...
    def _save_model(self):
        ...
        print("Save model of epoch {} at {}".format(
            self._counter, model_path))
...
class ExecutorCheckpoint(_Checkpoint):
    def __init__(self, root, period=None, explicit_period=False):
        ...
        self.latest_status_path = os.path.join(self.root,
                                               'latest_executor.status')
        self.latest_executor_path = os.path.join(self.root,
                                                 'latest_executor.pth')
    ...
    def on_work_begin(self, worker, **kwargs):
        ...
        biggest_epoch = 0
        if os.path.exists(self.latest_status_path):
            biggest_epoch, executor_path = _read_status(
                self.root, self.latest_status_path)

        if os.path.exists(self.last_status_path):
            biggest_epoch_, executor_path_ = _read_status(
                self.root, self.last_status_path)
            if biggest_epoch_ >= biggest_epoch:
                biggest_epoch = biggest_epoch_
                executor_path = executor_path_

        if executor_path is not None:
            print("Load executor of epoch {} from {}".format(
                biggest_epoch, executor_path))
            self._executor.load_state_dict(
                torch.load(executor_path, map_location='cpu'))
        self._counter = biggest_epoch
    ...
    def on_epoch_end(self, metric, **kwargs):
        ...
        if (self._counter % self.period) == 0:
            ...
            print("Save executor of epoch {} at {}".format(
                self._counter, executor_path))
            torch.save(self._executor_state_dict, executor_path)
            _write_status(self.latest_status_path, self._counter,
                          executor_path)

    def on_work_end(self):
        if self._has_updated:
            torch.save(self._executor_state_dict, self.last_executor_path)
            print("Save executor of last epoch {} at {}".format(
                self._counter, self.last_executor_path))
            _write_status(self.last_status_path, self._counter,
                          self.last_executor_path)
...
def get_best_result(best_epoch, result):
    ...
    for key, value in result.items():
        if len(result[key]) < best_epoch:
            raise Exception(
                "The length of data, {}, at best epoch, {}, cause conflict".
                format(len(result[key]), best_epoch))
...
class Checkpoint(Callback):
    def __init__(self, model_checkpoint, executor_checkpoint, recorder,
        ...
        self.callbacks = Callbacks(
            [self.model_checkpoint, self.executor_checkpoint, self.recorder])
        self._epoch_start = 0
    ...
    def weights_status_paths(self):
        paths = [self.executor_checkpoint.paths, self.model_checkpoint.paths]
        return paths
    ...
    def get_config(self, **kwargs):
        ...
        config['executor_checkpoint'] = self.executor_checkpoint.get_config(
            **kwargs)
        ...
        for path_dict in self.weights_status_paths:
            for dict_ in path_dict.values():
                ...
                if os.path.exists(status_path):
                    ...
                    new_status_path = '{}_epoch_{}.status'.format(
                        new_status_name, epoch)
                    new_weights_path = '{}_epoch_{}.pth'.format(
                        new_weights_name, epoch)
                    new_status_path = os.path.join(self.checkpoint_root,
                                                   new_status_path)
                    new_weights_path = os.path.join(self.checkpoint_root,
                                                    new_weights_path)
    ...
    def on_work_begin(self, worker, **kwargs):
        ...
        if self.model_checkpoint.save_best_weights:
            if os.path.exists(
                ...
                if best_record_epoch != best_status_epoch:
                    raise Exception(
                        "Inconsist best epoch between {} and {}".format(
                            best_record_epoch, best_status_epoch))
        ...
        with warnings.catch_warnings():
            warnings.simplefilter("ignore",
                                  category=CheckpointProtectedWarning)
    ...
    def on_work_end(self, **kwargs):
        ...
        if self.model_checkpoint.best_epoch is not None:
            ...
            if self._has_updated and self.best_record_path is not None:
                print("Save best result of epoch {}".format(
                    self.model_checkpoint.best_epoch))
                best_result = get_best_result(self.model_checkpoint.best_epoch,
                                              self.record)
                best_result = {
                    'best_epoch': self.model_checkpoint.best_epoch,
...
def build_checkpoint(root,only_recorder=False,
                     force_reset=False,monitor_target=None,
                     patience=None,period=None,is_train_mode=False,
                     warmup_epoch=None):
    if is_train_mode:
    ...
    if only_recorder:
        ...
        model_checkpoint = ModelCheckpoint(target=monitor_target,patience=patience,
                                           ...
                                           root=root,period=period,warmup_epoch=warmup_epoch)
