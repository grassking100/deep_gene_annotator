    ...
    for seq in seqs:
        for index, type_ in enumerate(seqs.ANN_TYPES):
            ...
            if seq.strand == PLUS:
                ...
                answer_vec[index][0:seq.length] += np.flip(
                    seq.get_ann(type_), 0)
...
def position_error(predict, answer, check_length=True):
    ...
    if predict.chromosome_id != answer.chromosome_id:
        ...
        raise Exception(err.format(predict.chromosome_id,
                                   answer.chromosome_id))
...
def partial_dependence_plot(df,
    ...
    plt.fill_between(sorted_keys,
                     sorted_values - sorted_stds,
                     sorted_values + sorted_stds,
                     alpha=.4,
                     label='std')
    plt.fill_between(sorted_keys,
...
def boxplot(df,
    ...
    keys = sorted(keys)
    values = [list(groups.get_group(key)['value']) for key in keys]
...
def plot_optim_history(study):
    df = study.trials_dataframe()
    x = np.array(df['number']) + 1
    values = df['value']
    max_value = None
    max_values = []
    for value in values:
    ...
    plt.plot(x, max_values, color='red', label='max value')
    plt.plot(x, values, '.', color='blue', label='value')
    plt.xlabel('Trial id')
    plt.ylabel('metric')
    plt.title('Metric at every trial')
    plt.legend()
