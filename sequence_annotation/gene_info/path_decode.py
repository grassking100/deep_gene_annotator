...
def find_path(seqs):
    #Check each cluster have same TSS
    def get_id(start,end):
        return "{}_{}".format(start,end)

    TSSs = list(set(int(mRNA['start']) for mRNA in seqs))
    if len(TSSs) != 1:
        raise Exception("TSSs sholud be same in each cluster.")
    CAs = list(set(int(mRNA['end']) for mRNA in seqs))
    if len(CAs) != 1:
        raise Exception("CAs sholud be same in each cluster.")

    length = CAs[0] - TSSs[0] + 1
...
def parse(bed_path,relation_path):
