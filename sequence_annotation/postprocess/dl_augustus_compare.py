...
def compare_df(lhs_df, rhs_df, lhs_name, rhs_name,
               filter_names=None, show_std=True):
    t_lhs_df = lhs_df.T
    t_rhs_df = rhs_df.T
    compared_result = {lhs_name: {}, rhs_name: {}}
    for name in t_lhs_df.columns:
        if filter_names is None or any(
                [filter_name in name for filter_name in filter_names]):
            l_values = list(t_lhs_df[name])
            r_values = list(t_rhs_df[name])
            compared_result[lhs_name]["mean_{}".format(
                name)] = np.mean(l_values)
            compared_result[rhs_name]["mean_{}".format(
                name)] = np.mean(r_values)
            if show_std:
                compared_result[lhs_name]["std_{}".format(
                    name)] = np.std(l_values)
                compared_result[rhs_name]["std_{}".format(
                    name)] = np.std(r_values)
...
def main(split_table_path, dl_revised_root, aug_project_root,
    ...
    aug_folder_names = get_augustus_folder_names(
        aug_folder_prefix, len(split_table))
    ...
    p_a_abs_diff_compare_result = compare_df(
        dl_p_a_abs_diff_results,
        aug_p_a_abs_diff_results,
        'DL',
        'Augustus',
        show_std=False)
    a_p_abs_diff_compare_result = compare_df(
        dl_a_p_abs_diff_results,
        aug_a_p_abs_diff_results,
        'DL',
        'Augustus',
        show_std=False)
    base_compare_result = compare_df(
        dl_base_results,
        aug_base_results,
        'DL',
        'Augustus',
        show_std=False)
    block_compare_result = compare_df(
        dl_block_results,
        aug_block_results,
        'DL',
        'Augustus',
        show_std=False)
    site_matched_result = compare_df(
        dl_site_matched_results,
        aug_site_matched_results,
        'DL',
        'Augustus',
        show_std=False)
    # Save result
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--dl_revised_root", required=True,
