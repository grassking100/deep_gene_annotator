...
class MetricBuilder():
    ...
    def add_loss(self,loss_type=None,weights=None,dynamic_weight_method=None,name=None):
        loss = Loss(name=name,weights=weights,
                    values_to_ignore=self._values_to_ignore,
                    type_=loss_type,
                    dynamic_weight_method=dynamic_weight_method)
        self._custom_objects["loss"]=loss
        return self
    def add_accuracy(self,name=None):
        acc = Accuracy(name=name,values_to_ignore=self._values_to_ignore)
        self._custom_objects["accuracy"]=acc
        return self
    def add_batch_count(self,name=None):
        metric = BatchCount(name=name)
