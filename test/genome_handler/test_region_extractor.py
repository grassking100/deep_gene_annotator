...
class TestRegionExtractor(SeqInfoTestCase):
    def test_region_extract(self):
        ...
        answer.init_space()
        chrom.init_space()
        chrom.add_ann("exon",1,0,1).add_ann("intron",1,2,4).add_ann("other",1,5,6).add_ann("sure",1,0,6)
        extractor = RegionExtractor()
        regions = extractor.extract(chrom)
        seqinfos = SeqInfoContainer()
        ids = ['region_1_exon_1','region_1_intron_1','region_1_other_1','region_1_sure_1']
        types = ['exon','intron','other','sure']
        for type_,id_,start,end in zip(types,ids,[0,2,5,0],[1,4,6,6]):
