...
class TestAltStatusConverter(unittest.TestCase):
    def _test(self,name,select_site_by_election):
        ...
        output = read_gff(output_gff_path).sort_values(['chr','strand','start','end','feature']).reset_index(drop=True)
        answer = read_gff(answer_gff_path).sort_values(['chr','strand','start','end','feature']).reset_index(drop=True)
