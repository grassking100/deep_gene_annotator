...
class TestPerformance(unittest.TestCase):
    def _test(self,name):
        ...
        main(predict_path,answer_path,region_table_path,saved_root,chrom_target='new_id')
        result = read_json(os.path.join(saved_root,'block_performance.json'))
        expect_result = read_json(os.path.join(data_root,'expect_part_block_performance.json'))
        for key,expect_val in expect_result.items():
            self.assertEqual(expect_val,result[key],"Something is wrong at {}".format(key))
            
        main(answer_path,answer_path,region_table_path,saved_root,chrom_target='new_id')
        result = read_json(os.path.join(saved_root,'block_performance.json'))
        expect_result = read_json(os.path.join(data_root,'expect_part_block_performance_for_full_correct.json'))
        for key,expect_val in expect_result.items():
            self.assertEqual(expect_val,result[key],"Something is wrong at {} of all correct test".format(key))
