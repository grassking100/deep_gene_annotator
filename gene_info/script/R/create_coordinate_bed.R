create_coordinate_bed <- function(safe_merged_gro_sites,safe_merged_cleavage_sites)
{
    #coordinate_consist_bed_path <- paste0(saved_root,str_with_time('coordinate_consist_with_','.bed'))
    #coordinate_consist_bed_with_gene_id_path <- paste0(saved_root,str_with_time('coordinate_consist_with_gene_id_','.bed'))
    #pure_coordinate_consist_bed_path <- paste0(saved_root,str_with_time('pure_coordinate_consist_','.bed'))
    ...
    coordinate_consist_gff <- data.frame(chr=paste('Chr',consist_data$chr,sep=''),
                                         ...
                                         score='.',strand=consist_data$strand,frame='.',
...
print(args)
if (length(args)!=3) {
  stop("Three arguments must be supplied (input file)", call.=FALSE)
}
saved_root <- args[1]
safe_merged_gro_sites_path <- args[2]
safe_merged_cleavage_sites_path <- args[3]
...
if(file.exists(coordinate_consist_bed_path)# & 
   ...
   #file.exists(pure_coordinate_consist_bed_path)
    ...
    #safe_merged_cleavage_sites_path <- paste0(saved_root,str_with_time('safe_merged_cleavage_sites_','.tsv'))  
    ...
    coordinate_consist_bed <- create_coordinate_bed(safe_merged_gro_sites,safe_merged_cleavage_sites)
