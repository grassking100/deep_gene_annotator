...
cp $separate_path/recurrent_cleaned.bed $result_root/result.bed
bash $script_root/bash/get_region.sh $result_root/result.bed $fai $upstream_dist $downstream_dist 

python3 $script_root/python/rename_bed.py -b $result_root/$result_merged.bed -p seq -s $result_root -r selected_region.bed
...
python3 $script_root/python/create_ann_region.py -m $result_root/result.bed -r $result_root/selected_region.bed \
